...
%             x_p(:,1)= Amat(:,:,1)*x0 + ft(:,1);
    ...
    %            
        ...
        %% Kalman Filter 
        function [x_p, Pe_p, x_u, Pe_u,Gn] = kalman_filter(A, C, Pv, Pw, Px0, y)
            %% DT Kalman Filter
        ...
        function [x_N, P_N,Ln,x_p, Pe_p, x_u, Pe_u] = kalman_smoother(A, C, Pv, Pw, Px0, y)
            %% kalman smoother
            N=size(y,2);
            [x_p, Pe_p, x_u, Pe_u] = kalman_filter(A, C, Pv, Pw, Px0, y);
...
%                     Dk(:,:,k)=W_u(:,:,k)*Tk(:,:,k)'*pinv(W_p(:,:,k)); %From deJong and MacKinnon 1988
                     Dk(:,:,k)=W_u(:,:,k)*Tk(:,:,k)'/(W_p(:,:,k)); %From deJong and MacKinnon 1988
